...
const ConectoReports: React.FC = () => {
    ...
    const fetchReportData = async () => {
        try {
            setLoading(true);
            const res = await axios.get(`${API_BASE_URL}/reports/conecto`, {
                params: { month: selectedMonth, year: selectedYear }
            });
            setReportData(res.data);
        } catch (err) {
            console.error('Conecto report fetch error:', err);
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchAllManualData = async () => {
        try {
            const res = await axios.get(`${API_BASE_URL}/admin/manual-data/conecto`);
    ...
    const fetchComputedMonthValues = async (month: number, year: number) => {
        try {
            const res = await axios.get(`${API_BASE_URL}/reports/conecto`, { params: { month, year } });
    ...
    const DonutChart = ({ title, value, detail, statsDetail, color = '#059669' }: any) => {
        const pieData = [
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200 flex flex-col items-center shadow-sm">
                ...
                <div className="relative w-32 h-32">
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            <Pie data={pieData} cx="50%" cy="50%" innerRadius={40} outerRadius={52} startAngle={90} endAngle={-270} dataKey="value" stroke="none">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                        ...
                        <span className="text-xl font-black" style={{ color }}>{Number(value || 0).toFixed(2).replace('.', ',')}</span>
                    </div>
                </div>
    ...
    if (loading && !reportData) return <div className="p-8 text-slate-700 text-center">Yükleniyor...</div>;
    ...
    return (
        <div className="p-6 space-y-6 max-w-[1600px] mx-auto text-slate-900">
            <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                <div>
                    <h1 className="text-2xl font-black text-slate-900 tracking-tight">CONECTO PDI RAPORU</h1>
                    <p className="text-slate-600 text-sm">Conecto şehir içi otobüs analizleri</p>
                </div>

                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 bg-slate-50 p-1.5 rounded-xl border border-slate-200">
                        <Calendar size={16} className="text-slate-500 ml-2" />
                        <select className="bg-transparent text-slate-900 px-2 py-1 focus:outline-none text-sm font-bold cursor-pointer" value={selectedMonth} onChange={(e) => setSelectedMonth(Number(e.target.value))}>
                            {MONTHS.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
                        </select>
                        <select className="bg-transparent text-slate-900 px-2 py-1 focus:outline-none text-sm font-bold border-l border-slate-200 cursor-pointer" value={selectedYear} onChange={(e) => setSelectedYear(Number(e.target.value))}>
                            {YEARS.map(y => <option key={y} value={y}>{y}</option>)}
                        </select>
                    </div>

                    <button onClick={openEditModal} className="flex items-center gap-2 px-5 py-2.5 bg-emerald-700 hover:bg-emerald-600 text-white rounded-xl transition-all font-bold text-sm">
                        <Edit2 size={16} />
                        Veri Düzenle
                    </button>
                </div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                <h3 className="text-lg font-black text-slate-900 mb-3 flex items-center gap-2">
                    <Info size={18} className="text-emerald-700" /> RAPOR ÖZETİ
                </h3>
                <p className="text-slate-700 leading-relaxed text-sm lg:text-base font-medium">
                    {selectedYear} {selectedMonthName} ayında PDI yapılan [CONECTO] araç sayısı
                    <span className="text-emerald-700 font-black mx-2 text-lg">{summary.current_month_vehicles}</span>
                    adettir. Son 12 ay ortalama hata oranı araç başı
                    <span className="text-emerald-700 font-black mx-2 text-lg">{summary.avg_12_month?.toFixed(2)}</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                <div className="lg:col-span-3 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden">
                    <div className="flex justify-between items-start mb-8 relative z-10">
                        <div>
                            <h3 className="text-lg font-bold text-slate-900 mb-1 tracking-tight">CONECTO HATA TRENDİ</h3>
                            <p className="text-slate-600 text-xs">Aylık araç başı hata oranı gelişimi</p>
                        </div>
                    ...
                    <div className="h-[420px] w-full relative z-10">
                        <ResponsiveContainer width="100%" height="100%">
                            <ComposedChart data={reportData?.monthly_trend} margin={{ top: 30, right: 30, left: 0, bottom: 10 }}>
                                <defs>
                                    <linearGradient id="barGradientCon" x1="0" y1="0" x2="0" y2="1">
                                ...
                                <YAxis yAxisId="left" stroke="#64748b" fontSize={11} tickLine={false} axisLine={false} domain={[0, (dataMax: number) => dataMax === 0 ? 50 : Math.ceil(dataMax * 1.3)]} />
                                <YAxis yAxisId="right" orientation="right" stroke="#dc2626" fontSize={11} tickLine={false} axisLine={false} domain={[0, (dataMax: number) => dataMax === 0 ? 5 : Math.max(5, Math.ceil(dataMax * 1.5))]} />
                                <Tooltip contentStyle={{ backgroundColor: '#ffffff', borderColor: '#cbd5e1', borderRadius: '12px', color: '#0f172a' }} itemStyle={{ fontSize: '13px', fontWeight: 600, color: '#0f172a' }} />
                                <Legend verticalAlign="top" align="right" wrapperStyle={{ fontSize: '11px', fontWeight: 700 }} />
                                <Bar yAxisId="left" name="Araç Sayısı" dataKey="vehicles" fill="url(#barGradientCon)" radius={[8, 8, 0, 0]} barSize={24} />
                                <Line yAxisId="right" name="Hata Oranı" type="monotone" dataKey="rate" stroke="#ef4444" strokeWidth={4} dot={{ r: 5, fill: '#ef4444', strokeWidth: 2, stroke: '#ffffff' }} activeDot={{ r: 8, strokeWidth: 0, fill: '#ef4444' }}>
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <DonutChart title="SON 12 AY AVG" value={summary.avg_12_month || 0} detail="Hareketli 12 Aylık Performans" statsDetail={`Araç: ${summary.last_12_cnt || 0} | Hata: ${summary.last_12_err || 0}`} color="#059669" />
                    <DonutChart title="MEVCUT YIL AVG" value={summary.current_year_rate || 0} detail={`${selectedYear} Yılı Güncel Durum`} statsDetail={`Araç: ${summary.curr_year_cnt || 0} | Hata: ${summary.curr_year_err || 0}`} color="#047857" />
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
                    <div className="p-8 border-b border-slate-200 flex justify-between items-center bg-slate-50">
                        <div>
                            <h2 className="text-xl font-black text-slate-900 tracking-tight">AYLIK VERİ TABLOSU</h2>
                            <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mt-1">Conecto Model Bazlı Dağılım</p>
                        </div>
                        <span className="bg-slate-100 text-slate-500 px-4 py-1.5 rounded-full text-[10px] font-black tracking-tighter border border-slate-200">SON 12 AY VERİSİ</span>
                    </div>
                    <div className="max-h-[420px] overflow-auto">
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-slate-50">
                                    <th className="px-6 py-4 text-left text-[10px] font-black text-slate-500 uppercase tracking-widest">AY / YIL</th>
                                    <th className="px-6 py-4 text-center text-[10px] font-black text-slate-500 uppercase tracking-widest">ARAÇ SAYISI</th>
                                    <th className="px-6 py-4 text-center text-[10px] font-black text-slate-500 uppercase tracking-widest">HATA SAYISI</th>
                                    <th className="px-6 py-4 text-right text-[10px] font-black text-slate-500 uppercase tracking-widest">HATA ORANI (AVG)</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-200">
                                {reportData?.monthly_trend.map((row: any) => (
                                    <tr key={`${row.year}-${row.month_num}`} className="hover:bg-slate-50 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col">
                                                <span className="text-slate-900 font-bold text-sm tracking-tight">{row.month_full}</span>
                                                <span className="text-slate-500 text-[10px] font-black tracking-widest">{row.year}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-center"><span className="text-slate-900 font-black text-base">{row.vehicles}</span></td>
                                        <td className="px-6 py-4 text-center"><span className="text-slate-700 font-bold">{row.errors}</span></td>
                                        <td className="px-6 py-4 text-right">
                                            <span className={`inline-flex items-center px-4 py-1.5 rounded-full text-xs font-black ${row.rate > 2.5 ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-emerald-50 text-emerald-700 border border-emerald-200'}`}>
                                                {row.rate.toFixed(2)}
                                            </span>
                                        </td>
                                    </tr>
                    ...
                    <div className="bg-white border border-slate-200 rounded-[2rem] w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
                        <div className="flex justify-between items-center p-8 border-b border-slate-200 bg-slate-50">
                            <div>
                                ...
                                <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mt-1">Conecto Raporu</p>
                            </div>
                        ...
                        <div className="p-8 space-y-6">
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <select className="w-full bg-white border-2 border-slate-200 rounded-2xl px-5 py-3.5 text-slate-900 focus:border-emerald-500 outline-none transition-all font-bold appearance-none cursor-pointer" value={editData.year} onChange={(e) => handleEditMonthYearChange('year', e.target.value)}>
                                        {YEARS.map(y => <option key={y} value={y}>{y}</option>)}
                                    </select>
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">RAPOR AYI</label>
                                    <select className="w-full bg-white border-2 border-slate-200 rounded-2xl px-5 py-3.5 text-slate-900 focus:border-emerald-500 outline-none transition-all font-bold appearance-none cursor-pointer" value={editData.month} onChange={(e) => handleEditMonthYearChange('month', e.target.value)}>
                                        {MONTHS.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
                                    </select>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <input type="number" className="w-full bg-white border-2 border-slate-200 rounded-2xl px-5 py-3.5 text-slate-900 focus:border-emerald-400 outline-none transition-all font-bold" value={editData.vehicle_count} onChange={(e) => setEditData({ ...editData, vehicle_count: e.target.value })} />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">HATA SAYISI</label>
                                    <input type="number" className="w-full bg-white border-2 border-slate-200 rounded-2xl px-5 py-3.5 text-slate-900 focus:border-red-400 outline-none transition-all font-bold" value={editData.error_count} onChange={(e) => setEditData({ ...editData, error_count: e.target.value })} />
                                </div>
                            </div>
                            <div className="flex gap-2 rounded-xl bg-slate-100 p-1 border border-slate-200">
                                <button type="button" onClick={() => setEditModalTab('monthly')} className={`flex-1 py-2 rounded-lg text-sm font-bold ${editModalTab === 'monthly' ? 'bg-white text-emerald-700 border border-emerald-200' : 'text-slate-600'}`}>Aylık Veri</button>
                                <button type="button" onClick={() => setEditModalTab('pie')} className={`flex-1 py-2 rounded-lg text-sm font-bold ${editModalTab === 'pie' ? 'bg-white text-emerald-700 border border-emerald-200' : 'text-slate-600'}`}>Pie Chart Düzenle</button>
                            </div>

                            {editModalTab === 'monthly' ? (
                                <>
                                    <div className="rounded-2xl bg-emerald-50 border border-emerald-200 px-4 py-3 text-sm text-emerald-900 font-semibold">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <textarea className="w-full bg-white border-2 border-slate-200 rounded-3xl px-5 py-4 text-slate-900 focus:border-blue-500 outline-none transition-all h-32 resize-none font-medium" value={editData.report_note} onChange={(e) => setEditData({ ...editData, report_note: e.target.value })} placeholder="Raporun altında görüntülenecek notları buraya ekleyin..." />
                                    </div>
                                </>
                            ) : (
                        ...
                        <div className="p-8 bg-slate-50 border-t border-slate-200 flex gap-4">
                            <button onClick={() => setShowEditModal(false)} className="flex-1 px-4 py-4 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded-2xl transition-all font-bold tracking-tight">İPTAL</button>
                            <button onClick={handleSaveManual} className="flex-1 px-4 py-4 bg-emerald-700 hover:bg-emerald-600 text-white rounded-2xl transition-all font-black tracking-tight flex items-center justify-center gap-2">
                                <Save size={20} />
