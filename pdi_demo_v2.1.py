...
class PDIApp:
    ...
    def show_dashboard(self):
        ...
        # Refresh Button
        tk.Button(filter_frame, text=" 🔄 YENİLE ", command=self.show_dashboard, bg="#eee", relief="flat", font=("Segoe UI", 9, "bold")).pack(side="left", padx=5)

        # DB bağlantısı - Cached connection kullan
        conn = self.get_db_connection()
        c = conn.cursor()
        
        # Filtreye göre sorgu hazırla
        filter_val = self.dashboard_month_filter.get()
        where_clause = ""
        params = []
        ...
        if filter_val != "TÜMÜ":
            try:
                ...
                where_clause = " WHERE ((substr(tarih_saat,4,2)=? AND substr(tarih_saat,7,4)=?) OR (substr(tarih_saat,6,2)=? AND substr(tarih_saat,1,4)=?))"
                params = [f"{m_idx:02d}", year, f"{m_idx:02d}", year]
            except: pass
        ...
        if cached:
            total_veh, total_err, c_tou, c_trv, c_con = cached
        else:
            # Toplam araç sayısı (Unique Şasi No)
            total_veh = c.execute(f"SELECT count(DISTINCT sasi_no) FROM pdi_kayitlari {where_clause}", params).fetchone()[0]
            # Toplam hata sayısı (Filtreye uyan tüm satırlar)
            total_err = c.execute(f"SELECT count(*) FROM pdi_kayitlari {where_clause}", params).fetchone()[0]
            
            def get_veh_count(arac):
                extra = " AND " if where_clause else " WHERE "
                q = f"SELECT count(DISTINCT sasi_no) FROM pdi_kayitlari {where_clause} {extra} arac_tipi='{arac}'"
                return c.execute(q, params).fetchone()[0]

            c_tou = get_veh_count('Tourismo')
            c_trv = get_veh_count('Travego')
            c_con = get_veh_count('Connecto')
            
            # Cache'e kaydet
            self.set_cached_data(cache_key, (total_veh, total_err, c_tou, c_trv, c_con))
    ...
    def show_trv_tou_report(self):
        try:
            ...
            if month is None:
                messagebox.showwarning("Uyarı", "Bu raporu görüntülemek için lütfen sağ üstten spesifik bir AY VE YIL seçiniz.\n(Tüm zamanlar filtresi bu rapor tipinde desteklenmemektedir.)")
            else:
    ...
    def show_top5_analysis(self):
        ...
        # Ensure values are never None/invalid
        month, year = self.get_selected_month_year()
        if month is None or year is None:
            tk.Label(container, text="Lütfen sağ üstten rapor dönemini seçiniz.", bg=COLOR_BG).pack(pady=50)
            return
    ...
    def show_connecto_top3(self):
        ...
        month, year = self.get_selected_month_year()
        data_res = self.get_connecto_top3(month, year)
    ...
    def calculate_monthly_data(self, arac_tipleri, month, year):
        ...
        for i in range(12):
            ...
            result = conn.execute(query, arac_tipleri + [f"{m:02d}", str(y), f"{m:02d}", str(y)]).fetchone()
        ...
        last_year_query = f"""SELECT COUNT(DISTINCT sasi_no), COUNT(*) FROM pdi_kayitlari 
                             WHERE arac_tipi IN ({placeholders}) 
                             AND (substr(tarih_saat,7,4) = ? OR substr(tarih_saat,1,4) = ?)"""
        ly_result = conn.execute(last_year_query, arac_tipleri + [str(year-1), str(year-1)]).fetchone()
        ...
        cy_query = f"""SELECT COUNT(DISTINCT sasi_no), COUNT(*) FROM pdi_kayitlari 
                       WHERE arac_tipi IN ({placeholders}) 
                       AND (substr(tarih_saat,7,4) = ? OR substr(tarih_saat,1,4) = ?)"""
        cy_result = conn.execute(cy_query, arac_tipleri + [str(year), str(year)]).fetchone()
        ...
        return {
            ...
            # Donutlar için gerekli alanlar (anahtar isimleri düzeltildi)
            'prev_year': year - 1,
    ...
    # HELPER: Top Hata analizi (Tümünü getirir)
    def get_top5_hata_analysis(self, arac_tipleri, month, year):
        ...
        if month is not None and year is not None:
            # Toplam araç sayılarını bir kez hesaplayalım (Performans için)
            trv_total = conn.execute(f"SELECT COUNT(DISTINCT sasi_no) FROM pdi_kayitlari WHERE arac_tipi='Travego' AND {date_filter}", 
                                    (f"{month:02d}", str(year), f"{month:02d}", str(year))).fetchone()[0]
            tou_total = conn.execute(f"SELECT COUNT(DISTINCT sasi_no) FROM pdi_kayitlari WHERE arac_tipi='Tourismo' AND {date_filter}", 
                                    (f"{month:02d}", str(year), f"{month:02d}", str(year))).fetchone()[0]
            ...
            prev_trv_total = conn.execute(f"SELECT COUNT(DISTINCT sasi_no) FROM pdi_kayitlari WHERE arac_tipi='Travego' AND {date_filter}", 
                                         (f"{prev_m:02d}", str(prev_y), f"{prev_m:02d}", str(prev_y))).fetchone()[0]
            prev_tou_total = conn.execute(f"SELECT COUNT(DISTINCT sasi_no) FROM pdi_kayitlari WHERE arac_tipi='Tourismo' AND {date_filter}", 
                                         (f"{prev_m:02d}", str(prev_y), f"{prev_m:02d}", str(prev_y))).fetchone()[0]
        else:
            # TÜM ZAMANLAR
            trv_total = conn.execute("SELECT COUNT(DISTINCT sasi_no) FROM pdi_kayitlari WHERE arac_tipi='Travego'").fetchone()[0]
            tou_total = conn.execute("SELECT COUNT(DISTINCT sasi_no) FROM pdi_kayitlari WHERE arac_tipi='Tourismo'").fetchone()[0]
            prev_trv_total = 0; prev_tou_total = 0; prev_m = None

        results = []
        for (hata_adi,) in top_hatalar: # Tüm aktif top hataları dön
            if month:
                trv_count = conn.execute(f"SELECT COUNT(*) FROM pdi_kayitlari WHERE arac_tipi='Travego' AND top_hata=? AND {date_filter}", 
                                        (hata_adi, f"{month:02d}", str(year), f"{month:02d}", str(year))).fetchone()[0]
                tou_count = conn.execute(f"SELECT COUNT(*) FROM pdi_kayitlari WHERE arac_tipi='Tourismo' AND top_hata=? AND {date_filter}", 
                                        (hata_adi, f"{month:02d}", str(year), f"{month:02d}", str(year))).fetchone()[0]
            else:
                trv_count = conn.execute("SELECT COUNT(*) FROM pdi_kayitlari WHERE arac_tipi='Travego' AND top_hata=?", (hata_adi,)).fetchone()[0]
                tou_count = conn.execute("SELECT COUNT(*) FROM pdi_kayitlari WHERE arac_tipi='Tourismo' AND top_hata=?", (hata_adi,)).fetchone()[0]
            ...
            diff = 0
            if month:
                prev_trv_count = conn.execute(f"SELECT COUNT(*) FROM pdi_kayitlari WHERE arac_tipi='Travego' AND top_hata=? AND {date_filter}", 
                                             (hata_adi, f"{prev_m:02d}", str(prev_y), f"{prev_m:02d}", str(prev_y))).fetchone()[0]
                prev_tou_count = conn.execute(f"SELECT COUNT(*) FROM pdi_kayitlari WHERE arac_tipi='Tourismo' AND top_hata=? AND {date_filter}", 
                                             (hata_adi, f"{prev_m:02d}", str(prev_y), f"{prev_m:02d}", str(prev_y))).fetchone()[0]
                
                prev_all_count = prev_trv_count + prev_tou_count
                prev_all_total = prev_trv_total + prev_tou_total
                prev_avg_rate = (prev_all_count / prev_all_total * 100.0) if prev_all_total > 0 else 0
                diff = avg_rate - prev_avg_rate
    ...
    def get_connecto_top3(self, month, year):
        ...
        # Filtreleme (YTD veya Tüm Zamanlar için)
        date_where = ""
        params = []
        if month is not None and year is not None:
             # YTD (Seçilen yıla göre ve seçilen aya kadar)
             date_where = """ AND (
                 (substr(tarih_saat,7,4) = ? AND CAST(substr(tarih_saat,4,2) AS INTEGER) <= ?) OR
                 (substr(tarih_saat,1,4) = ? AND CAST(substr(tarih_saat,6,2) AS INTEGER) <= ?)
             ) """
             params = [str(year), month, str(year), month]

        # Toplam Connecto hata sayısı
        total_errors = conn.execute(f"SELECT COUNT(*) FROM pdi_kayitlari WHERE arac_tipi='Connecto' {date_where}", params).fetchone()[0]
        
        # Top 3 hata
        top_hatalar = conn.execute(f"""SELECT top_hata, COUNT(*) as cnt FROM pdi_kayitlari 
                                     WHERE arac_tipi='Connecto' {date_where}
    ...
    def show_top_hata_detail(self, hata_adi, arac_tipleri):
        ...
        for i in range(12):
            ...
            for tip, label in [("Travego", "TRV"), ("Tourismo", "TOU")]:
                total = conn.execute(f"SELECT COUNT(DISTINCT sasi_no) FROM pdi_kayitlari WHERE arac_tipi=? AND {date_filter}",
                                    (tip, f"{m:02d}", str(y), f"{m:02d}", str(y))).fetchone()[0]
                hata = conn.execute(f"SELECT COUNT(*) FROM pdi_kayitlari WHERE arac_tipi=? AND top_hata=? AND {date_filter}",
                                   (tip, hata_adi, f"{m:02d}", str(y), f"{m:02d}", str(y))).fetchone()[0]
        ...
        # Legend - Tablonun altında ayrı frame olarak
        legend_frame = tk.Frame(win, bg=COLOR_WHITE, pady=10)
        legend_frame.pack(fill="x", padx=20)
        
        # TRV Legend
        trv_box = tk.Frame(legend_frame, bg="#0066cc", width=20, height=14)
        trv_box.pack(side="left", padx=(20, 5))
        trv_box.pack_propagate(False)
        tk.Label(legend_frame, text="TRV Hata Oranı", font=("Segoe UI", 9), bg=COLOR_WHITE).pack(side="left", padx=(0, 30))
        
        # TOU Legend
        tou_box = tk.Frame(legend_frame, bg="#ff8c00", width=20, height=14)
        tou_box.pack(side="left", padx=(0, 5))
        tou_box.pack_propagate(False)
        tk.Label(legend_frame, text="TOU Hata Oranı", font=("Segoe UI", 9), bg=COLOR_WHITE).pack(side="left")
    ...
    def load_list_data(self):
        ...
        sasi = self.f_sasi.get(); tip = self.f_tip.get(); alt = self.f_alt.get(); period = self.f_period.get()
        q = "SELECT id, bb_no, sasi_no, arac_tipi, tarih_saat, alt_grup, hata_konumu, top_hata, kullanici FROM pdi_kayitlari WHERE 1=1"
        p = []
        if sasi: q += " AND sasi_no LIKE ?"; p.append(f"%{sasi}%")
        if tip != "Tümü": q += " AND arac_tipi = ?"; p.append(tip)
        ...
        if period != "TÜMÜ":
            try:
                ...
                year_f = parts[1]
                m_idx = next((k for k, v in TURKISH_MONTHS.items() if v == m_name), 0)
                q += " AND ((substr(tarih_saat,4,2)=? AND substr(tarih_saat,7,4)=?) OR (substr(tarih_saat,6,2)=? AND substr(tarih_saat,1,4)=?))"
                p.extend([f"{m_idx:02d}", year_f, f"{m_idx:02d}", year_f])
            except: pass
        ...
        for r in rows:
            ...
            self.tree.insert("", "end", values=(r[0], r[1], r[2], r[3], date_d, r[5], r[6], r[7], r[8]))
    ...
    def delete_filtered_records(self):
        ...
        q = "DELETE FROM pdi_kayitlari WHERE 1=1"
        p = []
        if sasi: q += " AND sasi_no LIKE ?"; p.append(f"%{sasi}%")
        if tip != "Tümü": q += " AND arac_tipi = ?"; p.append(tip)
        if alt != "Tümü": q += " AND alt_grup = ?"; p.append(alt)
        if period != "TÜMÜ":
            try:
                parts = period.split()
                m_idx = next((k for k, v in TURKISH_MONTHS.items() if v == parts[0]), 0)
                year_f = parts[1]
                q += " AND ((substr(tarih_saat,4,2)=? AND substr(tarih_saat,7,4)=?) OR (substr(tarih_saat,6,2)=? AND substr(tarih_saat,1,4)=?))"
                p.extend([f"{m_idx:02d}", year_f, f"{m_idx:02d}", year_f])
            except: pass
        ...
        try:
            ...
            conn.commit()
            conn.close()
            messagebox.showinfo("Başarılı", f"{count} adet kayıt başarıyla silindi.")
            self.load_list_data()
        except Exception as e:
...
"""
    ...
    def render_imalat_trend_tab(self, parent, month, year):
        ...
        def update_view(event=None):
            ...
            def get_stat_local(crs, m, y):
                try:
                    total_pdi = crs.execute("SELECT COUNT(DISTINCT bb_no) FROM pdi_kayitlari WHERE CAST(substr(tarih_saat,4,2) AS INTEGER)=? AND CAST(substr(tarih_saat,7,4) AS INTEGER)=?", (m, y)).fetchone()[0]
                    total_im = crs.execute("SELECT COUNT(DISTINCT arac_no) FROM imalat_kayitlari WHERE CAST(substr(tarih,4,2) AS INTEGER)=? AND CAST(substr(tarih,7,4) AS INTEGER)=?", (m, y)).fetchone()[0]
                    return (total_im / total_pdi * 100.0) if total_pdi > 0 else 0
                except: return 0
                
            data_curr = [get_stat_local(c, m, y_target) for m in range(1, 13)]
            data_prev = [get_stat_local(c, m, y_base) for m in range(1, 13)]
    ...
    def render_imalat_top_errors_tab(self, parent, month, year):
        ...
        # YTD ve Ay verisi
        sql_ytd_arac = "SELECT COUNT(DISTINCT arac_no) FROM imalat_kayitlari WHERE CAST(substr(tarih,7,4) AS INTEGER)=? AND CAST(substr(tarih,4,2) AS INTEGER)<=?"
        arac_count = c.execute(sql_ytd_arac, (year, month)).fetchone()[0]
        
        recs = c.execute("SELECT hata_metni FROM imalat_kayitlari WHERE CAST(substr(tarih,7,4) AS INTEGER)=? AND CAST(substr(tarih,4,2) AS INTEGER)<=?", (year, month)).fetchall()
        total_hata = 0
        for r in recs:
            if r[0]: total_hata += len(r[0].strip().split('\n'))
            else: total_hata += 1
        ...
        # Data
        data_rows = c.execute("""SELECT top_hata, COUNT(*) as cnt FROM imalat_kayitlari 
                                WHERE CAST(substr(tarih,7,4) AS INTEGER)=? AND CAST(substr(tarih,4,2) AS INTEGER)<=?
                                AND top_hata != '' GROUP BY top_hata ORDER BY cnt DESC LIMIT 10""", (year, month)).fetchall()
