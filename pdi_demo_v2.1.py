...
ARAC_TIPI = ["Tourismo", "Conecto", "Travego"]
...
def init_db():
    ...
    # PDI Detayları tablosu (Dinamik hata listesi için)
    c.execute("""CREATE TABLE IF NOT EXISTS pdi_detaylari(
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        pdi_id INTEGER,
        aciklama TEXT,
        fotograf_yolu TEXT,
        FOREIGN KEY(pdi_id) REFERENCES pdi_kayitlari(id)
    )""")
    # Dropdown Yönetimi tablosu
    c.execute("""CREATE TABLE IF NOT EXISTS dropdown_items(
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        category TEXT,
        value_text TEXT,
        UNIQUE(category, value_text)
    )""")
    cols = ["grup_no", "parca_tanimi", "hata_tanimi", "musteri_sikayeti", "musteri_beklentisi", "musteri_geri_bildirimi", "musteri_teyidi", "musteri_talebi", "musteri_adi", "musteri_soyadi", "musteri_iletisim", "musteri_adresi", "duzenleyen", "top_hata", "hata_nerede"]
    for col in cols:
        try: c.execute(f"ALTER TABLE pdi_kayitlari ADD COLUMN {col} TEXT")
        except: pass
        
    # Başlangıç verilerini dropdown tablolarına ekle
    try:
        current_cats = [r[0] for r in c.execute("SELECT DISTINCT category FROM dropdown_items").fetchall()]
        if not current_cats:
            for item in ALT_GRUP: c.execute("INSERT OR IGNORE INTO dropdown_items(category, value_text) VALUES (?, ?)", ("ALT_GRUP", item))
            for item in HATA_KONUM: c.execute("INSERT OR IGNORE INTO dropdown_items(category, value_text) VALUES (?, ?)", ("HATA_KONUM", item))
            for item in ARAC_TIPI: c.execute("INSERT OR IGNORE INTO dropdown_items(category, value_text) VALUES (?, ?)", ("ARAC_TIPI", item))
            # Yeni Kategoriler için Varsayılan Değerler
            for item in ["İmalat", "Diğer"]: c.execute("INSERT OR IGNORE INTO dropdown_items(category, value_text) VALUES (?, ?)", ("HATA_NEREDE", item))
            for item in ["Boya", "Çizik", "Leke", "Kırık", "Eksik", "Trim Sesi", "Montaj Hatası"]: c.execute("INSERT OR IGNORE INTO dropdown_items(category, value_text) VALUES (?, ?)", ("TESPIT_ONERI", item))
    except: pass
    ...
    # Typo Migration: Connecto -> Conecto
    try:
        c.execute("UPDATE pdi_kayitlari SET arac_tipi='Conecto' WHERE arac_tipi='Connecto'")
    except: pass
    
    try:
        c.execute("UPDATE report_manual_data SET context_key='conecto' WHERE context_key='connecto'")
    except: pass
...
class PDIApp:
    ...
    def show_main_layout(self):
        ...
        self.create_sidebar_btn("📊  CONECTO RAPOR", self.show_connecto_report)
        self.create_sidebar_btn("📉  TOP 5 ANALİZ", self.show_top5_analysis)
        self.create_sidebar_btn("📉  CON. TOP 3", self.show_connecto_top3)
        ...
        if self.current_role == "admin": 
            ...
            self.create_sidebar_btn("⚙️  AYARLAR", self.open_settings_window)
    ...
    def _load_dashboard_data(self):
        ...
        c_con = df[df['arac_tipi'] == 'Conecto']['sasi_no'].nunique()
    ...
    def _render_dashboard(self, data):
        ...
        self.create_info_card(cards_frame, "CONECTO", str(c_con), "", 4)
    ...
    def show_connecto_report(self):
        try:
            ...
            messagebox.showerror("Hata", f"Conecto Rapor Hatası:\n{str(e)}")

    def _render_connecto_report(self, data):
        self.clear_content()
        self.create_report_header("CONECTO RAPORU", self.show_connecto_report)
        ...
        template_text = f"{year} {TURKISH_MONTHS[month]} ayında PDI yapılan [CONECTO] araç sayısı {data['current_month_vehicles']} adettir. Son 12 ay ortalama hata oranı araç başı {data['avg_12_month']:.2f} adet olup, {TURKISH_MONTHS[month]} ayı özelinde {data['current_month_rate']:.2f} adet olarak gerçekleşmiştir."
        ...
        # Zoom Butonu
        tk.Button(left_frame, text="🔍 Grafiği Tam Ekran Yap (Screenshot İçin)", command=lambda: self.zoom_chart(data['monthly_stats'], context_key="conecto"), 
                  bg="#f8f9fa", fg="#333", relief="flat", font=("Segoe UI", 9)).pack(anchor="ne", padx=10, pady=5)
        ...
        self.draw_combo_bar_line_chart(bar_canvas, data['monthly_stats'], context_key="conecto")
        ...
        self.draw_donut_section_vertical(right_frame, data, context_key="conecto")

    def show_top5_analysis(self, mode="YTD"):
        month, year = self.get_selected_month_year()
        
        # Eğer Ay/Yıl seçili değilse (TÜM ZAMANLAR), mode önemsizdir, hepsi listelenir.
        # Ancak fonksiyon imzasını koruyalım.
        
        self.run_async(
            self.get_top5_hata_analysis,
            lambda result: self._render_top5_analysis(result, month, year, mode),
            ["Tourismo", "Travego"], month, year, mode
        )

    def _render_top5_analysis(self, top5_data, month, year, mode="YTD"):
        self.clear_content()
        self.create_report_header("TÜM TOP HATALARIN ANALİZİ", lambda: self.show_top5_analysis(mode))
        
        # Üst kısım: YTD / MTD Seçimi (Sadece belirli bir ay seçiliyse anlamlı)
        if month is not None:
            t_frame = tk.Frame(self.content_area, bg=COLOR_BG)
            t_frame.pack(fill="x", padx=20, pady=(0, 10))
            
            tk.Label(t_frame, text="Görüntüleme Modu:", bg=COLOR_BG, font=FONT_BOLD).pack(side="left")
            
            def set_mode(m):
                # Yeniden yükle
                self.show_top5_analysis(m)

            b_ytd = tk.Button(t_frame, text="YTD (Yıl Başından Beri)", command=lambda: set_mode("YTD"), 
                              bg="#333" if mode=="YTD" else "#eee", fg="white" if mode=="YTD" else "#333", relief="flat")
            b_ytd.pack(side="left", padx=10)
            
            b_mtd = tk.Button(t_frame, text="MTD (Sadece Bu Ay)", command=lambda: set_mode("MTD"), 
                              bg="#333" if mode=="MTD" else "#eee", fg="white" if mode=="MTD" else "#333", relief="flat")
            b_mtd.pack(side="left", padx=5)
    ...
    def show_connecto_top3(self, mode="YTD"):
        try:
            self.clear_content()
            self.create_report_header("CONECTO TOP 3 ANALİZİ", lambda: self.show_connecto_top3(mode))
            ...
            if month is not None:
                ...
                b_ytd = tk.Button(t_frame, text="YTD (Yıl Başından Beri)", command=lambda: self.show_connecto_top3("YTD"), 
                                  bg="#333" if mode=="YTD" else "#eee", fg="white" if mode=="YTD" else "#333", relief="flat")
                b_ytd.pack(side="left", padx=10)
                
                b_mtd = tk.Button(t_frame, text="MTD (Sadece Bu Ay)", command=lambda: self.show_connecto_top3("MTD"), 
                                  bg="#333" if mode=="MTD" else "#eee", fg="white" if mode=="MTD" else "#333", relief="flat")
                b_mtd.pack(side="left", padx=5)

            self.run_async(
                lambda: self.get_connecto_top3(month, year, mode),
                lambda data_res: self._render_connecto_top3(data_res, month, year, mode)
            )
        except Exception as e:
            messagebox.showerror("Hata", f"Conecto Top 3 Raporu Hatası:\n{str(e)}")

    def _render_connecto_top3(self, data_res, month, year, mode="YTD"):
        ...
        if month is None:
            title = "TÜM ZAMANLAR Conecto Top 3 Hata Analizi"
            period_label = "TÜM"
        else:
            if mode == "MTD":
                title = f"{year} yılı {TURKISH_MONTHS.get(month, '')} ayı için Conecto Top 3 Hataları (MTD)"
                period_label = "MTD"
            else:
                title = f"{year} yılı {TURKISH_MONTHS.get(month, '')} ayına kadar Conecto Top 3 Hataları (YTD)"
                period_label = "YTD"
    ...
    def get_top5_hata_analysis(self, arac_tipleri, month, year, mode="YTD"):
        conn = self.get_db_connection()
        ...
        if month is not None and year is not None:
            if mode == "MTD":
                ...
                # Önceki ay (MTD karşılaştırması için)
                prev_m = (month - 2) % 12 + 1
                prev_y = year - (1 if month == 1 else 0)
                prev_mask = (df['dt'].dt.month == prev_m) & (df['dt'].dt.year == prev_y)
            
            else:
                # YTD (Yıl başından bu aya kadar)
                curr_mask = (df['dt'].dt.year == year) & (df['dt'].dt.month <= month)
                
                # Geçen Yıl Aynı Dönem (veya önceki ay? Genelde YTD için geçen yılın aynı dönemi mantıklıdır ama burada kod önceki ay ile kıyaslıyordu.
                # Kullanıcı isteği "YTD ve MTD olarak görebilme".
                # YTD karşılaştırması için bir önceki ayın YTD'si mi yoksa Geçen yılın YTD'si mi?
                # Basitlik ve mevcut yapı gereği: YTD karşılaştırması "Önceki Ayın YTD'si" olmaz.
                # Biz diff analizi için:
                # MTD -> Önceki Ay
                # YTD -> Geçen Yılın Aynı Dönemi (veya basitçe diff kolonunu boş geçebiliriz, ama kullanıcı 'stabil/artış' görmek istiyor)
                # Mevcut yapıyı bozmadan: YTD için, önceki ayın YTD'si mantıksız olur. Biz YTD karşılaştırmasını 'Geçen Yıl YTD' yapalım.
                
                prev_y = year - 1
                prev_mask = (df['dt'].dt.year == prev_y) & (df['dt'].dt.month <= month)
    ...
    def get_connecto_top3(self, month, year, mode="YTD"):
        conn = self.get_db_connection()
        
        # OBTİMİZE EDİLMİŞ (PANDAS)
        query = "SELECT top_hata, tarih_saat FROM pdi_kayitlari WHERE arac_tipi='Conecto'"
        df = pd.read_sql_query(query, conn)
        df['dt'] = pd.to_datetime(df['tarih_saat'], format='%d-%m-%Y', errors='coerce')
        df = df.dropna(subset=['dt'])
    ...
    def draw_combo_bar_line_chart(self, canvas, monthly_stats, context_key="general", fullscreen=False):
        ...
        def edit_combo():
            win = tk.Toplevel(self.root)
            win.title("Grafik Verilerini Düzenle (Fark Ekleme)")
            win.geometry("750x600")
            
            main_f = tk.Frame(win, padx=20, pady=20)
            main_f.pack(fill="both", expand=True)
            
            tk.Label(main_f, text="DİKKAT: Buraya gireceğiniz değerler, veritabanındaki değerin üzerine EKLENİR (veya çıkarılır).", font=("Segoe UI", 10, "bold"), fg="#dc3545").pack(pady=(0,5))
            tk.Label(main_f, text="Örnek: Araç Sayısı 100 ise ve siz '+5' yazarsanız grafikte 105 görünür. Yeni araç gelirse (101 olursa), grafikte 106 görünür.", font=("Segoe UI", 9), fg="#666").pack(pady=(0,10))
            ...
            tk.Label(scroll_f, text="DB Araç", font=FONT_BOLD, fg="#666").grid(row=0, column=1, padx=5)
            tk.Label(scroll_f, text="Araç Fark (+/-)", font=FONT_BOLD).grid(row=0, column=2, padx=5)
            tk.Label(scroll_f, text="DB Hata", font=FONT_BOLD, fg="#666").grid(row=0, column=3, padx=5)
            tk.Label(scroll_f, text="Hata Fark (+/-)", font=FONT_BOLD).grid(row=0, column=4, padx=5)
            tk.Label(scroll_f, text="Sonuç Oran", font=FONT_BOLD, fg="#dc3545").grid(row=0, column=5, padx=5)
            
            entries = []
            oran_labels = []
            
            def update_oran(idx, items):
                try:
                    # items = (db_arac_val, entry_arac_diff, db_hata_val, entry_hata_diff)
                    db_a = items[0]
                    diff_a = int(items[1].get()) if items[1].get() and items[1].get() not in ['-', '+'] else 0
                    
                    db_h = items[2]
                    diff_h = int(items[3].get()) if items[3].get() and items[3].get() not in ['-', '+'] else 0
                    
                    final_a = db_a + diff_a
                    final_h = db_h + diff_h
                    
                    oran = final_h / final_a if final_a > 0 else 0
                    items[4].config(text=f"%{oran*100:.2f}") # items[4] = label
                except:
                    items[4].config(text="Err")
            
            for i, m in enumerate(monthly_stats):
                r = i + 1
                tk.Label(scroll_f, text=m['ay'], font=("Segoe UI", 10, "bold")).grid(row=r, column=0, padx=5, pady=2)
                
                db_a_val = m['arac_sayisi']
                tk.Label(scroll_f, text=str(db_a_val), fg="#666").grid(row=r, column=1, padx=5)
                
                # Retrieve existing offset
                saved_off_a = self.get_manual_data("combo_offset", f"arac_off_{i}", context_key)
                e_a_off = ttk.Entry(scroll_f, width=8)
                e_a_off.insert(0, saved_off_a if saved_off_a else "0")
                e_a_off.grid(row=r, column=2, padx=5)
                
                db_h_val = m['hata_sayisi']
                tk.Label(scroll_f, text=str(db_h_val), fg="#666").grid(row=r, column=3, padx=5)
                
                saved_off_h = self.get_manual_data("combo_offset", f"hata_off_{i}", context_key)
                e_h_off = ttk.Entry(scroll_f, width=8)
                e_h_off.insert(0, saved_off_h if saved_off_h else "0")
                e_h_off.grid(row=r, column=4, padx=5)
                
                # Initial Calc
                try:
                    init_a = db_a_val + (int(saved_off_a) if saved_off_a else 0)
                    init_h = db_h_val + (int(saved_off_h) if saved_off_h else 0)
                    init_o = init_h / init_a if init_a > 0 else 0
                except: init_o = 0
                
                lbl_o = tk.Label(scroll_f, text=f"%{init_o*100:.2f}", width=8, fg="#dc3545", font=("Segoe UI", 10, "bold"))
                lbl_o.grid(row=r, column=5, padx=5)
                
                entries.append((e_a_off, e_h_off))
                oran_labels.append(lbl_o)
                
                # Bindings
                items = (db_a_val, e_a_off, db_h_val, e_h_off, lbl_o)
                # closure capture fix
                e_a_off.bind("<KeyRelease>", lambda e, idx=i, it=items: update_oran(idx, it))
                e_h_off.bind("<KeyRelease>", lambda e, idx=i, it=items: update_oran(idx, it))
                
            def save():
                for i, (ea, eh) in enumerate(entries):
                    off_a = ea.get()
                    off_h = eh.get()
                    # Sadece sayısal değerler kaydedilsin, boşsa 0
                    if not off_a: off_a = "0"
                    if not off_h: off_h = "0"
                    
                    self.save_manual_data("combo_offset", f"arac_off_{i}", context_key, off_a)
                    self.save_manual_data("combo_offset", f"hata_off_{i}", context_key, off_h)

                win.destroy()
                messagebox.showinfo("Başarılı", "Fark değerleri kaydedildi ve grafik güncellendi.")
                # Refresh logic calling parent refresh or just accept async refresh? 
                # The prompt implies immediate visual update, but since data comes from async loader which reads DB, 
                # we need to trigger re-draw. The caller 'redraw' might need to re-fetch data?
                # Actually, the chart is drawn with 'monthly_stats'. We need to re-call the draw function.
                # But 'monthly_stats' is passed as argument.
                # We should close the window and let user refresh, OR ideally refresh the chart in place.
                # Since we are inside draw function, we can't easily re-fetch DB data without callback.
                # User will likely hit refresh on dashboard or top selection.

            tk.Button(win, text="KAYDET VE KAPAT", command=save, bg="black", fg="white", font=FONT_BOLD, pady=10).pack(pady=10)
            scroll_f.update_idletasks(); canvas_edit.config(scrollregion=canvas_edit.bbox("all"))

        # Manual overrides (OFFSET LOGIC)
        processed_stats = []
        actual_max_count = 0
        actual_max_rate = 0
        ...
        for i, m in enumerate(monthly_stats):
            # Read Offsets
            off_a_str = self.get_manual_data("combo_offset", f"arac_off_{i}", context_key)
            off_h_str = self.get_manual_data("combo_offset", f"hata_off_{i}", context_key)
            
            try: off_a = int(off_a_str) if off_a_str else 0
            except: off_a = 0
            
            try: off_h = int(off_h_str) if off_h_str else 0
            except: off_h = 0
            
            # Calculate Final Values
            final_arac = m['arac_sayisi'] + off_a
            final_hata = m['hata_sayisi'] + off_h
            # Negatif değerleri engelle
            if final_arac < 0: final_arac = 0
            if final_hata < 0: final_hata = 0
            
            final_oran = final_hata / final_arac if final_arac > 0 else 0
            
            processed_stats.append({
                'ay': m['ay'], 
                'arac_sayisi': final_arac, 
                'hata_sayisi': final_hata, 
                'hata_orani': final_oran
            })
            actual_max_count = max(actual_max_count, final_arac, final_hata)
            actual_max_rate = max(actual_max_rate, final_oran)
    ...
    def show_list_view(self):
        ...
        # Hata Nerede Filter
        tk.Label(filter_frame, text="Hata Nerede Giderildi:", bg=COLOR_WHITE).grid(row=0, column=4, sticky="w", padx=5)
        # Dynamic filter values
        conn = self.get_db_connection()
        hn_vals = [r[0] for r in conn.execute("SELECT value_text FROM dropdown_items WHERE category='HATA_NEREDE' ORDER BY value_text").fetchall()]
        if not hn_vals: hn_vals = ["İmalat", "Diğer"]
        self.f_nerede = ttk.Combobox(filter_frame, values=["TÜMÜ"] + hn_vals, state="readonly"); self.f_nerede.current(0); self.f_nerede.grid(row=1, column=4, padx=5)

        btn_f = tk.Button(filter_frame, text="FİLTRELE", command=self.load_list_data, bg="black", fg="white", relief="flat", padx=10)
        btn_f.grid(row=1, column=5, padx=15)
        
        tk.Button(filter_frame, text="EXCEL İNDİR", command=self.export_excel_with_images, bg="#007bff", fg="white", relief="flat", padx=10).grid(row=1, column=6, padx=5)
        
        if self.current_role == "admin":
            tk.Button(filter_frame, text="DÖNEMİ SİL", command=self.delete_filtered_records, bg="#dc3545", fg="white", relief="flat", padx=10).grid(row=1, column=7, padx=5)
            
        table_frame = tk.Frame(self.content_area, bg=COLOR_WHITE); table_frame.pack(fill="both", expand=True)
        # Yeni sütun: Üretim Yeri (Hata Nerede)
        cols = ("ID", "BB No", "Şasi No", "Araç", "Tarih", "Alt Grup", "Hata", "Top Hata", "Hata Nerede Giderildi", "Ekleyen")
        ...
        for c in cols: 
            ...
            width = 100
            if c in ["Hata", "Top Hata"]: width = 120
        ...
        # DOUBLE CLICK handler -> Open details
        ...
        tk.Button(action_bar, text="DÜZENLE / DETAY", command=self.open_edit_window, bg="#ffc107", relief="flat", width=15, height=2).pack(side="top", pady=2)
    ...
    def load_list_data(self):
        ...
        nerede = self.f_nerede.get()
        
        conn = self.get_db_connection()
        # Include hata_nerede in query
        query = "SELECT id, bb_no, sasi_no, arac_tipi, tarih_saat, alt_grup, hata_konumu, top_hata, kullanici, hata_nerede FROM pdi_kayitlari"
        df = pd.read_sql_query(query, conn)
        ...
        if nerede and nerede != "TÜMÜ":
            # Handle potential None values smoothly
            df['hata_nerede'] = df['hata_nerede'].fillna("Diğer")
            df = df[df['hata_nerede'] == nerede]
        ...
        for _, row in df.iterrows():
            # Tarih kısmını sadece gün olarak göster
            ...
            vals = [
                ...
                row['top_hata'], row['hata_nerede'], row['kullanici']
            ]
    ...
    def popup_window(self, title, data=None):
        top = tk.Toplevel(self.root); top.title(title); top.geometry("1100x850"); top.configure(bg=COLOR_BG)
        ...
        def get_vals(cat, default=None):
            try:
                conn = sqlite3.connect(DB_NAME)
                res = [r[0] for r in conn.execute("SELECT value_text FROM dropdown_items WHERE category=? ORDER BY value_text", (cat,)).fetchall()]
                conn.close()
                return res if res else (default if default else [])
            except: return default if default else []

        arac_tipleri = get_vals("ARAC_TIPI", ARAC_TIPI)
        alt_gruplar = get_vals("ALT_GRUP", ALT_GRUP)
        hata_konumlari = get_vals("HATA_KONUM", HATA_KONUM)
        hata_nerede_list = get_vals("HATA_NEREDE", ["İmalat", "Diğer"])
        tespit_onerileri = get_vals("TESPIT_ONERI", [])

        info_lbl = tk.Label(main_frame, text="ARAÇ VE HATA BİLGİLERİ", font=("Segoe UI", 11, "bold"), bg=COLOR_WHITE, fg="#888"); info_lbl.pack(anchor="w", pady=(0,10))
        grid_frame = tk.Frame(main_frame, bg=COLOR_WHITE); grid_frame.pack(fill="x", pady=(0, 10))
        ...
        def add_field(lbl, r, c, val=None, combo=None, is_date=False):
            ...
            if is_date:
                f = tk.Frame(grid_frame, bg=COLOR_WHITE); f.grid(row=r+1, column=c, sticky="ew", padx=10, pady=(2, 10))
                w = ttk.Entry(f, font=FONT_NORMAL); w.pack(side="left", fill="x", expand=True)
                if val: w.insert(0, val)
                tk.Button(f, text="📅", command=lambda: open_dt_dialog(w), relief="flat", bg="#eee", width=3).pack(side="right", padx=5)
            elif combo: 
                w = ttk.Combobox(grid_frame, values=combo, state="readonly", font=FONT_NORMAL)
                if val: w.set(val)
                elif combo: w.current(0)
        ...
        def open_dt_dialog(entry):
            d = AdvancedCalendarDialog(top, entry.get())
            if d.result: entry.delete(0, "end"); entry.insert(0, d.result)
        ...
        e_arac = add_field("Araç Tipi", 0, 2, data[3] if data else "", arac_tipleri)
        ...
        e_alt = add_field("Alt Grup", 2, 2, data[5] if data else "", alt_gruplar)
        
        # Satır 4: Hata Nerede Giderildi & Hata Konumu & Top Hata
        tk.Label(grid_frame, text="Hata Nerede Giderildi?", bg=COLOR_WHITE, font=FONT_NORMAL, fg="#333").grid(row=4, column=0, sticky="w", padx=10)
        tk.Label(grid_frame, text="Hata Nerede Giderildi?", bg=COLOR_WHITE, font=FONT_NORMAL, fg="#333").grid(row=4, column=0, sticky="w", padx=10)
        cb_hata_nerede = ttk.Combobox(grid_frame, values=hata_nerede_list, state="readonly", font=FONT_NORMAL)
        cb_hata_nerede.grid(row=5, column=0, sticky="ew", padx=10, pady=(2, 10))
        if data and len(data) > 14: cb_hata_nerede.set(data[14] if data[14] else "TUM")
        else: cb_hata_nerede.current(0)

        tk.Label(grid_frame, text="Hata Konumu", bg=COLOR_WHITE, font=FONT_NORMAL, fg="#333").grid(row=4, column=1, sticky="w", padx=10)
        cb_hata_konumu = ttk.Combobox(grid_frame, values=hata_konumlari, font=FONT_NORMAL) # Editable combo
        cb_hata_konumu.grid(row=5, column=1, sticky="ew", padx=10, pady=(2, 10))
        if data and data[7]: cb_hata_konumu.set(data[7])
        
        # Top Hata (Her zaman görünür olsun artık)
        tk.Label(grid_frame, text="Top Hata (İmalat için)", bg=COLOR_WHITE, font=FONT_NORMAL, fg="#333").grid(row=4, column=2, sticky="w", padx=10)
        conn = sqlite3.connect(DB_NAME); th_list = [r[0] for r in conn.execute("SELECT hata_adi FROM top_hatalar WHERE aktif=1").fetchall()]; conn.close()
        cb_top_hata = ttk.Combobox(grid_frame, values=[""]+th_list, state="readonly", font=FONT_NORMAL)
        cb_top_hata.grid(row=5, column=2, sticky="ew", padx=10, pady=(2, 10))
        if data and data[13]: cb_top_hata.set(data[13])
        
        # --- TESPİT DETAYLARI (DYNAMIC LIST) ---
        sep = tk.Frame(main_frame, bg="#eee", height=2); sep.pack(fill="x", pady=10)
        tk.Label(main_frame, text="TESPİT DETAYLARI (Yazın ve Enter'a basın)", font=("Segoe UI", 11, "bold"), bg=COLOR_WHITE, fg="#888").pack(anchor="w")
        
        details_container = tk.Frame(main_frame, bg=COLOR_WHITE)
        details_container.pack(fill="both", expand=True, pady=5)
        
        # Input Area
        input_row = tk.Frame(details_container, bg="#f0f0f0", padx=10, pady=10)
        input_row.pack(fill="x")
        
        e_new_detay = ttk.Combobox(input_row, values=tespit_onerileri, font=FONT_NORMAL)
        e_new_detay.pack(side="left", fill="x", expand=True, padx=(0, 10))
        
        # List Area
        list_area = tk.Frame(details_container, bg="white", bd=1, relief="solid")
        list_area.pack(fill="both", expand=True, pady=10)
        
        canvas = tk.Canvas(list_area, bg="white"); vsb = ttk.Scrollbar(list_area, orient="vertical", command=canvas.yview)
        scroll_frame = tk.Frame(canvas, bg="white"); canvas.create_window((0,0), window=scroll_frame, anchor="nw")
        canvas.configure(yscrollcommand=vsb.set); canvas.pack(side="left", fill="both", expand=True); vsb.pack(side="right", fill="y")
        
        scroll_frame.bind("<Configure>", lambda e: canvas.configure(scrollregion=canvas.bbox("all")))
        
        # Data Structure for Details: list of dicts {text: str, photo: str}
        # data: id, bb, sasi, arac, is, alt, tespit, konum, foto, tarih, user, duzenleyen, grup, top, hata_nerede
        detail_items = []
        
        if data:
             conn = sqlite3.connect(DB_NAME)
             try:
                 det_rows = conn.execute("SELECT aciklama, fotograf_yolu FROM pdi_detaylari WHERE pdi_id=?", (data[0],)).fetchall()
                 if det_rows:
                     for dr in det_rows: detail_items.append({"text": dr[0], "photo": dr[1]})
                 elif data[6]: # Eski usul tespitler
                     lines = data[6].split('\n')
                     for l in lines: 
                         if l.strip(): detail_items.append({"text": l.strip(), "photo": data[8] if data[8] else ""}) 
             except: pass
             conn.close()
        ...
        def render_details():
            ...
            for idx, item in enumerate(detail_items):
                ...
                txt_lbl = tk.Label(row, text=f"• {item['text']}", bg="white", font=FONT_NORMAL, anchor="w", justify="left")
                txt_lbl.pack(side="left", fill="x", expand=True)
                
                has_photo = bool(item['photo']) and os.path.exists(item['photo'])
                p_color = "#27ae60" if has_photo else "#95a5a6"
                p_text = "📷 Fotoğraf Var" if has_photo else "📷 Fotoğraf Ekle"
                
                def attach_photo(i=idx):
                    f = filedialog.askopenfilename(filetypes=[("Resimler", "*.jpg *.png *.jpeg")]); 
                    if f: 
                        detail_items[i]['photo'] = f
                        render_details()

                def view_photo(path):
                    if path and os.path.exists(path):
                        try: os.startfile(path)
                        except: messagebox.showerror("Hata", "Dosya açılamadı.")
                
                # Show View button only if photo exists
                if has_photo:
                     tk.Button(row, text="GÖSTER", command=lambda p=item['photo']: view_photo(p), bg=COLOR_BLUE, fg="white", font=("Segoe UI", 8), relief="flat").pack(side="left", padx=2)

                tk.Button(row, text=p_text, command=lambda i=idx: attach_photo(i), bg=p_color, fg="white", font=("Segoe UI", 8), relief="flat").pack(side="left", padx=5)
                tk.Button(row, text="🗑", command=lambda i=idx: remove_item(i), bg="#e74c3c", fg="white", relief="flat", width=3).pack(side="left")

        def remove_item(idx):
            detail_items.pop(idx)
            render_details()

        def add_detail_entry(event=None):
            val = e_new_detay.get().strip()
            if not val: return
            detail_items.append({"text": val, "photo": ""})
            e_new_detay.delete(0, "end")
            render_details()
            
        e_new_detay.bind("<Return>", add_detail_entry)
        tk.Button(input_row, text="EKLE", command=add_detail_entry, bg="#333", fg="white", relief="flat").pack(side="right")
        
        render_details()
        
        def save():
            bb = e_bb.get(); sasi = e_sasi.get()
            hata_nerede = cb_hata_nerede.get()
            h_konum = cb_hata_konumu.get()
            top_hata = cb_top_hata.get()
            tarih_val = e_tarih.get()
            
            if not sasi: messagebox.showwarning("Eksik", "Şasi No giriniz."); return
            
            # Tespitleri string olarak birleştir (eski uyumluluk için)
            all_text = "\n".join([item["text"] for item in detail_items])
            
            # Ana fotoyu belirle (ilk item'ın fotosu olsun, varsa)
            main_photo_path = ""
            for item in detail_items:
                if item["photo"]: main_photo_path = item["photo"]; break
            
            # Fotoğrafları kopyala ve path güncelle
            for item in detail_items:
                if item["photo"] and os.path.exists(item["photo"]):
                     if not item["photo"].startswith(PHOTO_DIR):
                         try:
                             ext = os.path.splitext(item["photo"])[1]
                             fname = f"{sasi}_{datetime.now().strftime('%Y%m%d%H%M%S')}_{os.urandom(2).hex()}{ext}"
                             dest = os.path.join(PHOTO_DIR, fname)
                             shutil.copy2(item["photo"], dest)
                             item["photo"] = dest
                             if main_photo_path == item["photo"]: main_photo_path = dest
                         except: pass

            conn = sqlite3.connect(DB_NAME); c = conn.cursor()
            try:
                pdi_id = data[0] if data else None
                if data: 
                    c.execute("""UPDATE pdi_kayitlari SET bb_no=?, sasi_no=?, arac_tipi=?, is_emri_no=?, alt_grup=?, tespitler=?, hata_konumu=?, fotograf_yolu=?, tarih_saat=?, duzenleyen=?, top_hata=?, hata_nerede=? WHERE id=?""", 
                             (bb, sasi, e_arac.get(), e_isemri.get(), e_alt.get(), all_text, h_konum, main_photo_path, tarih_val, self.current_user, top_hata, hata_nerede, data[0]))
                else: 
                    c.execute("""INSERT INTO pdi_kayitlari (bb_no, sasi_no, arac_tipi, is_emri_no, alt_grup, tespitler, hata_konumu, fotograf_yolu, tarih_saat, kullanici, top_hata, hata_nerede) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""", 
                             (bb, sasi, e_arac.get(), e_isemri.get(), e_alt.get(), all_text, h_konum, main_photo_path, tarih_val, self.current_user, top_hata, hata_nerede))
                    pdi_id = c.lastrowid
                
                # Detayları Kaydet
                c.execute("DELETE FROM pdi_detaylari WHERE pdi_id=?", (pdi_id,))
                for item in detail_items:
                    c.execute("INSERT INTO pdi_detaylari (pdi_id, aciklama, fotograf_yolu) VALUES (?,?,?)", (pdi_id, item["text"], item["photo"]))
                
                # İmalata Gönderilecek mi?
                if hata_nerede == "İmalat":
                    # Mükerrer kontrol et
                    exist = c.execute("SELECT id FROM imalat_kayitlari WHERE arac_no=? AND tarih=?", (sasi, tarih_val)).fetchone()
                    if not exist:
                        c.execute("INSERT INTO imalat_kayitlari (arac_no, tarih, adet, top_hata, hata_metni, kullanici, olusturma_tarihi) VALUES (?,?,?,?,?,?,?)",
                                 (sasi, tarih_val, 1, top_hata, all_text, self.current_user, datetime.now().strftime("%d-%m-%Y %H:%M")))
    ...
    def open_settings_window(self):
        ...
        # Tabs
        self.create_settings_tab(notebook, "Top Hatalar", "top_hatalar")
        self.create_settings_tab(notebook, "Hata Konumları", "HATA_KONUM")
        self.create_settings_tab(notebook, "Alt Gruplar", "ALT_GRUP")
        self.create_settings_tab(notebook, "Araç Tipleri", "ARAC_TIPI")
        self.create_settings_tab(notebook, "Hata Nerede Giderildi", "HATA_NEREDE")
        self.create_settings_tab(notebook, "Tespit Önerileri", "TESPIT_ONERI")
        
    def create_settings_tab(self, notebook, title, category_or_table):
        frame = tk.Frame(notebook, bg=COLOR_WHITE, padx=20, pady=20)
        notebook.add(frame, text=f"  {title}  ")
        
        # List
        list_frame = tk.Frame(frame, bg="#f9f9f9", bd=1, relief="solid")
        list_frame.pack(fill="both", expand=True, pady=(0, 10))
        
        lb = tk.Listbox(list_frame, font=FONT_NORMAL, bd=0, bg="#f9f9f9", selectbackground="#ddd", selectforeground="black")
        ...
        def load_items():
            lb.delete(0, "end")
            conn = sqlite3.connect(DB_NAME)
            if category_or_table == "top_hatalar":
                rows = conn.execute("SELECT hata_adi FROM top_hatalar WHERE aktif=1 ORDER BY hata_adi").fetchall()
                for r in rows: lb.insert("end", r[0])
            else:
                rows = conn.execute("SELECT value_text FROM dropdown_items WHERE category=? ORDER BY value_text", (category_or_table,)).fetchall()
                for r in rows: lb.insert("end", r[0])
            conn.close()
        ...
        # Add
        add_frame = tk.Frame(frame, bg=COLOR_WHITE)
        add_frame.pack(fill="x", pady=10)
        e_new = ttk.Entry(add_frame, font=FONT_NORMAL)
        ...
        def add_item():
            ...
            try:
                if category_or_table == "top_hatalar":
                    conn.execute("INSERT INTO top_hatalar (hata_adi, aktif, olusturma_tarihi) VALUES (?, 1, ?)", (val, datetime.now().strftime("%d-%m-%Y")))
                else:
                    conn.execute("INSERT INTO dropdown_items (category, value_text) VALUES (?, ?)", (category_or_table, val))
                conn.commit()
                e_new.delete(0, "end")
                load_items()
            except Exception as e:
        ...
        tk.Button(add_frame, text="EKLE", command=add_item, bg=COLOR_GREEN, fg="white", relief="flat", padx=15).pack(side="right")
        ...
        def delete_item():
            ...
            if not sel: return
            val = lb.get(sel[0])
            if messagebox.askyesno("Sil", f"'{val}' silinsin mi?"):
                conn = sqlite3.connect(DB_NAME)
                if category_or_table == "top_hatalar":
                     conn.execute("UPDATE top_hatalar SET aktif=0 WHERE hata_adi=?", (val,))
                else:
                     conn.execute("DELETE FROM dropdown_items WHERE category=? AND value_text=?", (category_or_table, val))
                conn.commit()
                conn.close()
                load_items()

        tk.Button(frame, text="SEÇİLİ ÖĞEYİ SİL", command=delete_item, bg=COLOR_RED, fg="white", relief="flat").pack(fill="x")
...
"""
    ...
    def render_imalat_top_errors_tab(self, parent, month, year):
        ...
        tk.Label(grid_container, text=f"{year} YTD İMALAT TOP HATA ANALİZİ", font=("Segoe UI", 12, "bold"), bg=COLOR_WHITE).pack(pady=10)
